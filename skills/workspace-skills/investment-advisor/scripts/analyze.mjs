...
async function fullAnalysis(symbol) {
    // 使用东方财富 API 获取报价 + 分析
    // 分别获取，允许部分失败
    let technicalFull = null;
    let fundamentalFull = null;
    let quote = null;
    
    try {
        technicalFull = await technical.getFullTechnicalAnalysis(symbol);
    } catch (e) {
        console.error('技术分析失败:', e.message);
    }
    
    try {
        fundamentalFull = await fundamental.getFullFundamentalAnalysis(symbol);
    } catch (e) {
        console.error('基本面分析失败:', e.message);
    }
    
    try {
        quote = await technical.fetchQuote(symbol);
    } catch (e) {
        console.error('行情数据失败:', e.message);
    }

    // 如果全部失败
    if (!technicalFull && !fundamentalFull && !quote) {
    ...
    const techScore = technicalFull?.score?.score || 50;
    const fundScore = fundamentalFull?.score?.score || 50;
    const sentScore = calculateSentimentScore(
        fundamentalFull?.sentiment || {score: 0, overall: 'neutral'}, 
        fundamentalFull?.analystRating || {mean: 'hold'}
    );
    ...
    const concerns = [];
    if (fundamentalFull?.profitability?.roe?.value < 10 && fundamentalFull?.profitability?.roe?.value > 0) concerns.push(`ROE偏低(${fundamentalFull.profitability.roe.value}%)`);
    ...
    const action = generateAction(
        ...
        technicalFull?.movingAverages?.support || 0, technicalFull?.movingAverages?.resistance || 0,
        fundamentalFull?.valuation?.fairValue?.estimated || 0, technicalFull?.atr || 0
    );
    ...
    return {
        ...
        technical: {
            ...
            support: technicalFull?.movingAverages?.support || 0,
            resistance: technicalFull?.movingAverages?.resistance || 0,
            indicators: {
        ...
        fundamental: {
            ...
            fairValue: fundamentalFull?.valuation?.fairValue?.estimated || 0,
            details: {
        ...
        sentiment: {
            score: sentScore, newsSentiment: fundamentalFull?.sentiment?.overall || 'neutral',
            analystRating: fundamentalFull?.analystRating?.mean || 'hold',
