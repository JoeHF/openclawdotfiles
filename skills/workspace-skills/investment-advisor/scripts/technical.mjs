...
function getSecId(symbol) {
    const s = String(symbol).replace(/\.(SH|SZ|sh|sz)$/, '');  // 去掉后缀
    if (/^\d{6}$/.test(s)) {
...
export async function fetchQuote(symbol) {
    ...
    // 尝试使用K线API获取最新数据（备用方案）
    async function fetchFromKline() {
        const klineUrl = `https://push2his.eastmoney.com/api/qt/stock/kline/get?secid=${secid}&fields1=f1,f2,f3,f4,f5,f6&fields2=f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61&klt=101&fqt=1&end=20500101&lmt=1`;
        
        try {
            const response = await fetch(klineUrl);
            if (!response.ok) throw new Error('K线API失败');
            const data = await response.json();
            ...
            // 解析最新一根K线: "日期,开盘,收盘,最高,最低,成交量,成交额,振幅,涨跌幅,涨跌额,换手率"
            const latestKline = data.data.klines[0].split(',');
            
            return {
                code: data.data.code,
                name: data.data.name,
                price: parseFloat(latestKline[2]),
                high: parseFloat(latestKline[3]),
                low: parseFloat(latestKline[4]),
                open: parseFloat(latestKline[1]),
                prevClose: null,  // K线API不提供昨收
                volume: parseInt(latestKline[5]),
                turnover: parseFloat(latestKline[6]),
                volumeRatio: null,
                marketCap: null,
                floatMarketCap: null,
                pe: null,
                pb: null,
                turnoverRate: parseFloat(latestKline[10])
            };
        } catch (e) {
            throw new Error(`K线API也失败: ${e.message}`);
        }
    }
    ...
    try {
        ...
        // 如果主API失败，尝试K线API
        console.log('主API失败，尝试K线API...');
        return fetchFromKline();
    }
}
